function initengahan(){
  ...
  // Simpan konten pesan 11-25 dan pastikan mereka tersembunyi
  for (let i = 11; i <= 25; i++) {
    const elemen = document.getElementById("pesan" + i);
    if (elemen) {
      window["vketik" + i] = elemen.innerHTML;
      elemen.innerHTML = "";
      elemen.style.opacity = "0";
      elemen.style.display = "block"; // Pastikan display block
    }
  }  
  ...
  function aktipesan10(){
    ...
    fungsi=2;setTimeout(tombol,4000); // Tambah delay untuk tombol
    setTimeout(munculPesanLanjutan, 1000); // Panggil lebih cepat
  }
  ...
  // Fungsi untuk menampilkan pesan 11-25 dengan perbaikan
  function munculPesanLanjutan() {
    const totalPesan = 15; // pesan11 - pesan25
    const delayAwal = 1000; // Delay awal dikurangi
    const jedaAntarPesan = 2000; // Jeda antar pesan dikurangi
  
    for (let i = 0; i < totalPesan; i++) {
      const index = 11 + i;
      const idPesan = "pesan" + index;
      const elemen = document.getElementById(idPesan);
      const isi = window["vketik" + index];
      
      if (elemen && isi) {
        setTimeout(() => {
          // Pastikan elemen terlihat dulu
          elemen.style = "opacity:1;transform:scale(1);transition:all .7s ease;display:block;position:relative;";
          ...
          new TypeIt(`#${idPesan}`, {
            ...
            afterComplete: function () {
              ...
              // Ganti stiker setiap beberapa pesan
              if (index % 3 === 0) {
                ftganti = (ftganti + 1) % 6;
                ftmuncul();
              }
